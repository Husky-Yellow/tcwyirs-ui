...
const generateMockFileConfigs = (count: number) => {
  ...
  for (let i = 1; i <= count; i++) {
    const storage = Random.pick(storageList)
    configs.push({
      id: Random.integer(1, 1000),
      name: Random.pick(['本地存储', '阿里云OSS', '七牛云', '腾讯云COS', 'MinIO']),
      storage: storage,
      master: Random.boolean(),
      config: {
        ...
        accessKey: Random.string('upper', 16),
        accessSecret: Random.string('lower', 32)
      },
...
const generateMockFiles = (count: number) => {
  ...
  for (let i = 1; i <= count; i++) {
    files.push({
      ...
      configId: Random.integer(1, 8),
      name: `${Random.word(5, 10)}.${Random.pick(['png', 'jpg', 'pdf', 'zip', 'txt'])}`,
      path: `/upload/${Random.date('yyyy/MM/dd')}/${Random.guid()}.png`,
      url: `https://cdn.example.com/${Random.guid()}.png`,
      type: Random.pick(fileTypes),
      size: Random.integer(1024, 10485760),
      createTime: Random.datetime('yyyy-MM-dd HH:mm:ss')
    })
  }
  return files
...
const mockConfigs: MockConfig[] = [
  ...
  // 上传文件
  {
    ...
    response: (): ApiResponse<string> => {
      return {
        code: 0,
        data: `https://cdn.example.com/${Random.guid()}.png`,
        msg: '上传成功'
      }
    }
  }
