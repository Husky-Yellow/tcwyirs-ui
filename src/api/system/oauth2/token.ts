import request from '@/config/axios'

export interface OAuth2TokenVO {
  id: number
  accessToken: string
  refreshToken: string
  userId: number
  userType: number
  clientId: string
  createTime: Date
  expiresTime: Date
}

export interface OAuth2TokenPageReqVO extends PageParam {
  userId?: number
  userType?: number
  clientId?: string
}

// 查询 token列表
export const getAccessTokenPage = (params: OAuth2TokenPageReqVO) => {
  return request.get<PageResult<OAuth2TokenVO[]>>({ url: '/system/oauth2-token/page', params })
}

// 删除 token
export const deleteAccessToken = (accessToken: string) => {
  return request.delete<void>({ url: '/system/oauth2-token/delete?accessToken=' + accessToken })
}
